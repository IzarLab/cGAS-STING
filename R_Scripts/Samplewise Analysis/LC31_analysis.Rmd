...
### Load dataset
cell_bender_mat <- Read_CellBender_h5_Mat(file_name = "/Users/zackbrodtman/Documents/jobwork/CUIMC/JG02/LC/LC31/feature_bc_matrix_bent_filtered.h5")
...
doublet_rate <- doublet_rate_df$multiple_rate[which.min(abs(doublet_rate_df$cells_recovered - dim(seu_raw@assays$RNA)[2]))]*0.01
...
sweep.list <- paramSweep(seu, PCs = 1:30)
...
# Run doubletFinder
seu <- doubletFinder(seu, PCs = 1:30, pN = 0.25, pK = pK, nExp = nExp.poi.adj, reuse.pANN = FALSE)
...
b_cell_types <- c("Plasma.cells",	"Activated.B.cells",
                  ...
                  "Memory.B.cells",	"Naive.B.cells",	"Transitional.B.cells",	"Large.pre-B.cells",
...
myeloid_types <- c("MacrophagesM1", 'MacrophagesM2',
  "M1_Macrophage_Polarization",	"M2_Macrophage_Polarization", "Mono_CD16", "Mono_CD16_C1qa",  "Mono_Apobec3b", "Mono_Apobec3a", "Mono_CD14_Antiviral", "Mono_CD14", 'Mono_CD14_Inflam',	
  "MonoT_Doublet",
  "Monocyte",	"Inflammatory_monocytes",	"resident_monocytes"
  )
...
DimPlot(seu, label = T) + theme(legend.position="bottom")

# 3 CAF
# 7 Fibroblasts
# 6 Cycling Endothelial/B cells
# 1 cycling b
# 4,5 endothelial
# 0,2 tumour
...
new.cluster.ids <- c('Tumor/Endothelial', 'Cycling B Cells', 'Tumor/Endothelial', 'CAF','Endothelial','Endothelial','Cycling Endothelial/B Cells','Fibroblasts')
...
```{r}
seu_cnv <- readRDS("/Users/zackbrodtman/Documents/jobwork/CUIMC/LKB1/LC31/LC31_cnv.rds")

FeaturePlot(seu_cnv, features = "cnv_avg")


seu_cnv@meta.data$cnv_avg
```
...
saveRDS(seu, file = 'LC31_seu_processed.rds')

### write pdf reports
pdf(file = "LC31_plots.pdf")
